  ...
  <div>
    <el-main>
        <el-button-group style="margin-bottom: 10px; margin-top: 10px;">
          <el-button @click="dialogFormVisible = true" type="primary" plain :icon="Plus">新建入库单</el-button>
        </el-button-group>
        <el-table
          v-loading="loading"
          ...
          <el-table-column type="expand">
            <template #default="scope">
              <el-table
                ...
                border
                highlight-current-row>
                <el-table-column prop="stockInId" label="入库单号"></el-table-column>
                ...
                <el-table-column label="单价（￥）">
                  <template #default="scope">
                    <el-form-item
                      v-if="scope && scope.$index >= 0"
                      label=" "
                      :prop="'stockInDetailList.' + scope.$index + '.price'"
                      :rules="rules.price">
...
export default {
    ...
    data: function() {
        return {
            ...
            form: {
              remark: null,
              stockInDetailList: [{goodsTypeId: null, goodsId: null, amount: null, unit: null, price: null}],
            },
            rules: {
              goodsTypeId: [
    ...
    methods: {
        loadTableData(current, size) {
            let _this = this;
            this.$http.get('/stock-in/details', { params: {current: current, size: size} }).then(function (response) {
        ...
        addRow() {
            this.form.stockInDetailList.push({goodsTypeId: null, goodsId: null, amount: null, unit: null, price: null});
        },
        deleteRow(index) {
